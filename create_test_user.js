...
async function createTestUser() {
  const email = 'sam_manager@nextask.com';
  const password = 'password123';
  const fullName = 'Manager Sam';
  ...
  if (error) {
    if (error.message.includes('already registered')) {
      console.log('User already exists. Proceeding to verification phase.');
      process.exit(0);
    }
  ...
  console.log('Please log in as Owner to approve this user.');
}
